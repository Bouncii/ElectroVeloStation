<?php
namespace App\Http\Controllers;

use App\Models\Station;
use App\Models\Person;
use App\Models\Schedule;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Inertia\Inertia;
use Illuminate\Support\Facades\DB;
use App\Models\Reservation;
use App\Models\Bike;

class StationReservationController extends Controller
{

    //On récupère les stations
    public function show()
    {
        return Inertia::render('reservation', [
            'allStations' => Station::all(),
            'peopleDb' => Auth::check()
                ? Person::where('user_id', Auth::id())->get()
                : [],
            'schedules' => Schedule::all(),
        ]);
    }



    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $validated = $request->validate([
            'start_date'=> 'required|date|after:now',
            'end_date' => 'required|date|after:start_date',
            'pickup_station_id' => 'required|exists:stations,id',
            'return_station_id' => 'required|exists:stations,id',
            'user_id'=> 'nullable|exists:users,id',
            'attributions'=> 'required|array|min:1',
            'attributions.*.person_id' => 'nullable|exists:people,id',

            'attributions.*.first_name' => 'required_without:attributions.*.person_id|string',
            'attributions.*.last_name' => 'required_without:attributions.*.person_id|string',
            'attributions.*.age' => 'nullable|integer',
            'attributions.*.required_bike_size' => 'nullable|integer',
            'attributions.*.user_id' => 'nullable|exists:users,id',

        ]);

        
        $allBikesAssigned = true;

        DB::transaction(function () use ($validated, &$allBikesAssigned) {
            $reservation = Reservation::create([
                'start_date' => $validated['start_date'],
                'end_date' => $validated['end_date'],
                'pickup_station_id' => $validated['pickup_station_id'],
                'return_station_id' => $validated['return_station_id'],
                'user_id' => $validated['user_id'],
                'status' => 'confirmed',
            ]);

            $assignedBikeIds = []; 

            foreach ($validated['attributions'] as $attr) {
                if (!empty($attr['person_id'])) {

                // Personne existante
                $person = Person::findOrFail($attr['person_id']);

                } else {

                    // Nouvelle personne
                    $person = Person::create([
                        'nom' => $attr['first_name'],
                        'prenom' => $attr['last_name'],
                        'age' => $attr['age'] ?? null,
                        'taille' => $attr['required_bike_size'] ?? null,
                    ]);
                }
                
                $bike = Bike::where('size', $person->bike_size)
                    ->whereNotIn('id', $assignedBikeIds) 
                    ->availableAtStationOn(
                        $validated['pickup_station_id'], 
                        $validated['start_date'], 
                        $validated['end_date']
                    )
                    ->first();

                if ($bike) {
                    $assignedBikeIds[] = $bike->id;
                    $bikeId = $bike->id;
                } else {
                    $allBikesAssigned = false;
                    $bikeId = null;
                }

                $reservation->attributions()->create([
                    'person_id' => $person->id,
                    'bike_id'   => $bikeId
                ]);
            }
        });

        if ($allBikesAssigned ) {
            $message = 'Réservation confirmée avec tous les vélos !' ;
        }else{
            $message = 'Réservation enregistrée, mais certains vélos manquent (statut : En attente).';
        }

        return redirect()->route('reservations.index')->with('success', $message);
    }
}